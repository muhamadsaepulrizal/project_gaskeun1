...
<div class="page-header flex items-center justify-between">
    <div>
        ...
        <h1 class="page-title">Edit Data Nelayan</h1>
    </div>
    <a href="{{ route('disperindag.nelayans.index') }}" class="btn-secondary">← Kembali</a>
</div>
<div class="card p-8 max-w-xl">
    <form action="{{ route('disperindag.nelayans.update', $item->id) }}" method="POST" class="space-y-5">
        @csrf @method('PUT')
        <div>
            ...
            <div class="relative mt-1">
                <select name="penduduk_id" required class="select-field pr-10">
                    ...
                    @foreach($penduduks as $opt)
                        <option value="{{ $opt->id }}" style="background:#141E2E;" {{ $item->penduduk_id == $opt->id ? 'selected' : '' }}>{{ $opt->nik }} - {{ $opt->nama_lengkap }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <input type="text" name="jenis_kapal" value="{{ old('jenis_kapal', $item->jenis_kapal) }}" required class="input-field mt-1">
            @error('jenis_kapal')<p class="mt-1.5 text-xs" style="color:#F43F5E;">{{ $message }}</p>@enderror
        </div>
        <div>
            <label class="label-field">Alat Tangkap</label>
            <input type="text" name="alat_tangkap" value="{{ old('alat_tangkap', $item->alat_tangkap) }}" required class="input-field mt-1">
